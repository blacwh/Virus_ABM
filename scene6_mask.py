...
class Particle:
    ...
    def __init__(self, x, y, vx, vy, radius, status, color, mask):
...
class Simulation:
    ...
    def __init__(self, n, infectn, radius, iterations, mask_wr, mask_eff):
        ...
        self.mask_efficiency = mask_eff
        self.mask_wear_rate = mask_wr

        self.init_particles(n, infectn, radius)

    def place_particle(self, rad, status, color, mask):
        ...
        particle = self.ParticleClass(x, y, vx, vy, rad, status, color, mask)
    ...
    def init_particles(self, n, infectn, radius):
        ...
        self.particles = []
        maskn = int(n * self.mask_wear_rate)


        for i in np.arange(self.infectn):
            self.place_particle(radius, STATUS[1], COLORS[1], mask=False)
        for i in np.arange(self.n - len(self.particles)):
            if i < maskn:
                self.place_particle(radius, STATUS[0], COLORS[0], mask=True)
            else:
                self.place_particle(radius, STATUS[0], COLORS[0], mask=False)
    ...
    def infectionDetect(self, a1, a2):
        ...
        if a1.status == STATUS[0] and a2.status == STATUS[1]:
            ...
            if a1.mask or a2.mask:
                infect_rate = self.infect_ratio * (1 - self.mask_efficiency)
            else:
                infect_rate = self.infect_ratio
            if infect_test < infect_rate:
                a1.status = STATUS[1]
                a1.color = COLORS[1]
                a1.draw(self.ax[0])
    ...
    def animate(self, i):
        ...
        if len(self.xdata) == self.iterations:
            self.create_splot(100, 200, self.ydata1, self.ydata2, self.ydata3, self.ydata4, 'Scenario-6')
            # with open('s6d.txt','w') as f:
            #     f.write('test: ')
            #     f.write('\nx: ')
...
if __name__ == '__main__':
    '''
    scenario 6 (mask)
    para1: mask wearing rate (70%, 100% default)
    para2: mask efficiency (60%, 80% default)
    '''
    nparticles = 200
    radius = 0.012
    infectn = 2
    iterations = 100                                         
    mask_wr = float(input("Please input the mask wearing rate (distinguishable rate is recommended): "))
    mask_eff = float(input("Please input the mask efficiency (above 50% is recommended): "))
    sim = Simulation(nparticles, infectn, radius, iterations, mask_wr, mask_eff)
