...
class Simulation:
    ...
    def __init__(self, n, infectn, radius, iterations, ld_threshold, ro_threshold, lockdown=False):
        ...
        self.ro_threshold = ro_threshold
    ...
    def advance_animation(self):
        ...
        if self.current_infections > int(self.ld_threshold * self.total_num) :
            self.dt = 0.01
        elif self.current_infections < int(self.ro_threshold * self.total_num) :
            self.dt = 0.15
    ...
    def create_splot(self, iterations, tnumber, line1, line2, line3, line4, title):
        ...
        ax.axhline(y=60, ls="--", c="navy",label='lockdown')
        ax.axhline(y=6, ls='-.', c="purple",label='release')
    ...
    def animate(self, i):
        ...
        if len(self.xdata) == self.iterations:
            self.create_splot(100, 200, self.ydata1, self.ydata2, self.ydata3, self.ydata4, 'Scenario-5')
    ...
    def setup_animation(self):
        ...
        self.ax[1].axhline(y=int(self.ld_threshold * self.total_num), ls="--", c="black",label='lockdown')
        self.ax[1].axhline(y=int(self.ro_threshold * self.total_num), ls="--", c="black", label='release')
...
if __name__ == '__main__':
    '''
    scenario 5 (partially lockdown)
    para1: lockdown threshold (20% default)
    para2: reopen threshold (5% default)
    '''
    nparticles = 200
    radius = 0.012
    infectn = 2
    iterations = 100
    ld_threshold = float(input("Please input the lockdown threshold (10%--30% is recommended): "))
    ro_threshold = float(input("Please input the reopen threshold (3%--10% is recommended): "))
    sim = Simulation(nparticles, infectn, radius, iterations, ld_threshold, ro_threshold)
